...
# -*- coding: utf-8 -*-
"""
Created on Sun Jan 29 17:44:50 2017
...
order = tf.placeholder(tf.int32, [None, 5], "order")              # [batch_size x 5]

batch_size = tf.shape(story)[0]

sentences = [tf.reshape(x, [batch_size, -1]) for x in tf.split(1, 5, story)]  # 5 times [batch_size x max_length]
for i in range(len(sentences)):
    sentences[i].set_shape([None,max_sent_len])
orders = tf.split(1,5,order)
...
output,_ =  tf.nn.seq2seq.embedding_rnn_seq2seq(sentences,orders, cell,vocab_size,state_size,256) 
...
# prediction function
output_test,_ =  tf.nn.seq2seq.basic_rnn_seq2seq(hs, cell=cell) 
output_test = output_test[4]
output_test = tf.reshape(output_test,[-1,state_size])
logits_flat_test = tf.add(tf.matmul(output_test,weights['out']),biases['out'])   # [batch_size x 5*target_size]
logits_test = logits = tf.reshape(logits_flat_test, [-1, 5, target_size])        # [batch_size x 5 x target_size]
...
with tf.Session() as sess:
    ...
    for epoch in range(10):
        ...
        dev_feed_dict = {story: dev_stories}
