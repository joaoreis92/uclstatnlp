...
###### Layers ######
cell_fw = tf.nn.rnn_cell.LSTMCell(state_size,state_is_tuple=True)
cell_bw = tf.nn.rnn_cell.LSTMCell(state_size,state_is_tuple=True)
...
output,_ = tf.nn.bidirectional_dynamic_rnn(cell_fw,cell_bw,h,dtype=tf.float32,sequence_length=tf.to_int64(tf.tile([5],[tf.to_int32(tf.shape(h)[0])]))) 
