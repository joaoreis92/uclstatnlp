...
loss = tf.reduce_sum(tf.nn.sparse_softmax_cross_entropy_with_logits(logits, order) +
    ...
    lambda_l2 * tf.nn.l2_loss(weights['h1'])+
    lambda_l2 * tf.nn.l2_loss(biases['h1'])+
...
with tf.Session() as sess:
    ...
    for epoch in range(10):
        ...
        for i in range(n // BATCH_SIZE):
            ...
            #random_batch_index = random.sample(range(len(train_stories)),BATCH_SIZE)
            #inst_story = np.asarray([train_stories[k] for k in random_batch_index])
            #inst_order = np.asarray([train_orders[k] for k in random_batch_index])
            ...
            _, current_loss, ss = sess.run([opt_op, loss,sentences_embedded], feed_dict=feed_dict)
            #print(len(ss))
            #print(ss[0].shape)
